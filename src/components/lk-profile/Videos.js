...
import preview2 from "../../image/preview-2.png"
import ticketText from "../../image/tickettext.png";
...
export default function Videos(props){
    ...
    //
    //     // Открываем попап с выбранным видео-роликом
    //     const openPopup = (event) => {
    //         event.preventDefault();
    //
    //         const videoUrl = videoLink.getAttribute('href');
    //
    //         // Устанавливаем ссылку на видео в плеере
    ...
    //
    //     return () => {
    //         videoLink.removeEventListener('click', openPopup);
                    ...
                    <div className={'main-items main-items-profile'}>
                        ...
                        <div className="container-profile">
                            ...
                            <div className="right-column-profile">
                                ...
                                <div className={'right-second-videos'}>
                                    <div className={'content-from-left'}>
                                        {/* Render videos */}
                                        {Object.entries(videos).map(([date, videoList]) => (
                                            <div key={date} className={'videos-winners-block-left-new'}>
                                                <div className={'asd'}>
                                                    <p className={'right-second-videos-p'}>{date}</p>
                                                </div>
                                                <div className={'videos-row-videos'}>
                                                    {videoList.map((video, index) => (
                                                        <div key={index} className={'prizes-items11-gifts-videos-new'}>
                                                            <div className={'gifts-content-videos-box11'}>
                                                                <div className={'full-videos'}>
                                                                    <a href="#" onClick={() => openPopup1(video)}
                                                                       className="video-link">
                                                                        <img className="video-preview" src={preview1}
                                                                             alt="Video Preview"/>
                                                                    </a>
                                                                </div>
                                                                <div className={'text-under-videos-new'}>
                                                                    <a onClick={() => openPopup1(video)}
                                                                       className={'text-under-videos-a'}>
                                                                        {video.title}
                                                                        <br/>
                                                                        {video.prize}
                                                                    </a>
